...
# Перевірка наявності ffmpeg для роботи з mp3
...
class AudioCleanerApp:
    def __init__(self, root):
        ...
        self.load_btn = tk.Button(root, text='Завантажити аудіофайл', command=self.load_audio)
        self.load_btn.pack(pady=10)

        self.process_btn = tk.Button(root, text='Обробити', command=self.process_audio, state=tk.DISABLED)
        self.process_btn.pack(pady=10)

        self.save_btn = tk.Button(root, text='Зберегти результат', command=self.save_audio, state=tk.DISABLED)
        self.save_btn.pack(pady=10)

    def load_audio(self):
        filetypes = [('Audio Files', '*.wav *.mp3 *.flac *.ogg'), ('All Files', '*.*')]
        filename = filedialog.askopenfilename(title='Виберіть аудіофайл', filetypes=filetypes)
        if filename:
            try:
                self.audio = AudioSegment.from_file(filename)
                self.filename = filename
                self.process_btn.config(state=tk.NORMAL)
                messagebox.showinfo('Успіх', f'Файл {os.path.basename(filename)} завантажено!')
            except Exception as e:
                messagebox.showerror('Помилка', f'Не вдалося завантажити файл: {e}')

    def process_audio(self):
        if not self.audio:
            messagebox.showerror('Помилка', 'Спочатку завантажте аудіофайл!')
            return
        try:
            # 1. Обрізка тиші
            trimmed = self.trim_silence(self.audio)
            # 2. Конвертація в numpy для noisereduce
            ...
            messagebox.showinfo('Готово', 'Обробка завершена!')
        except Exception as e:
            messagebox.showerror('Помилка', f'Помилка обробки: {e}')

    def trim_silence(self, audio, silence_thresh=-40, chunk_size=10):
        # Обрізка тиші на початку та в кінці
        dBFS = audio.dBFS
    ...
    def save_audio(self):
        if not self.processed_audio:
            messagebox.showerror('Помилка', 'Немає обробленого аудіо для збереження!')
            return
        ...
        if save_path:
            try:
                ...
                messagebox.showinfo('Успіх', f'Файл збережено: {save_path}')
            except Exception as e:
